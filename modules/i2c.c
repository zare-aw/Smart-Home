...
#define PLL_FACTOR        FCCLK/FOSC
#define VPBDIV_FACTOR     1
 ...
 *      10h START condition transmitted
 ...
 *****************************************************************************/
sint8
i2cGetChar(uint8  mode,
           uint8* pData)
{
  sint8 retCode = I2C_CODE_OK;
 ...
 *****************************************************************************/
sint8
i2cPutCharAndWait(uint8 data)
{
  sint8 retCode;
  
  retCode = i2cPutChar(data);
  while(retCode == (sint8)I2C_CODE_BUSY)
    retCode = i2cPutChar(data);
         ...
         uint16 len)
{
  ...
  do
  {
    ...
    //Transmit MSB of extra word (if wanted)
  	...
  	{
      retCode = i2cPutCharAndWait((uint8)(extra >> 8));
      if(retCode != I2C_CODE_OK)
        break;
    }
    ...
    //Transmit LSB of extra work (if wanted)
   	...
   	{
      retCode = i2cPutCharAndWait((uint8)(extra & 0xff));
      if(retCode != I2C_CODE_OK)
        break;
	  }
...
i2cPoll(uint8 addr)
{
  sint8 retCode  = I2C_CODE_OK;
  uint8 status   = 0;
  volatile uint8 deviceReady = FALSE;
        ...
        uint16 len)
{
  ...
  //write SLA+R
  retCode = i2cPutChar(addr);
  while(retCode == (sint8)I2C_CODE_BUSY){retCode = i2cPutChar(addr);}
    ...
    for(i = 1; i <= len; i++ )
    {
      ...
      while(1)
      {
        ...
        if((status == 0x40 ) || (status == 0x48 ) || (status == 0x50 ))
        {
          ...
          while(retCode == (sint8)I2C_CODE_EMPTY)
            retCode = i2cGetChar(I2C_MODE_READ, pBuf);
          pBuf++;
          break;
        }
